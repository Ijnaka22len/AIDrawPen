...
static void on_data_available(uint8_t int_id, void *ctx)
{
  ...
  if (head_ptr >= ACC_SEQUENCE_LENGTH) {
    init_done = true;
  }
...
void send_acc_data_via_ble(imu_data_t* data)
{
  sl_status_t sc;
  uint8_t acc_data_bytes[6];

  // 16-bit x, y, z data to bytes
  acc_data_bytes[0] = data->x & 0xFF;
  acc_data_bytes[1] = (data->x >> 8) & 0xFF;
  acc_data_bytes[2] = data->y & 0xFF;
  acc_data_bytes[3] = (data->y >> 8) & 0xFF;
  acc_data_bytes[4] = data->z & 0xFF;
  acc_data_bytes[5] = (data->z >> 8) & 0xFF;

  sc = sl_bt_gatt_server_notify_all(gattdb_acc_databyte,
                                    sizeof(acc_data_bytes),
                                    acc_data_bytes);
